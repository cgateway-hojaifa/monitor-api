...
import * as service from "@/modules/health-monitoring/health-monitoring.service";

// ── Status (last-30-min heartbeats for every monitor) ─────────────────────────
export const status = catchAsync(async (_req: Request, res: Response) => {
  const data = await service.statusWindow();
...
// ── Events ──────────────────────────────────────────────────────────────────
export const events = catchAsync(async (req: Request, res: Response) => {
  const { data, total, page, perPage } = await service.listEvents(Number(req.params.id), req.query);
  res.status(httpStatus.OK).json({ success: true, data, total, page, perPage });
});
